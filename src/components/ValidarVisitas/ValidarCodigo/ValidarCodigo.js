...
export function ValidarCodigo(props) {
  ...
  const [isNavigating, setIsNavigating] = useState(false); // Estado para manejar la navegación

  // Manejar la detección de navegación y mostrar el toast antes de salir
  useEffect(() => {
    const handleRouteChangeStart = (url) => {
      if (resultado !== null && !isNavigating) {
        setShowToast(true); // Mostrar el toast personalizado
        router.events.emit('routeChangeError'); // Cancelar la navegación
        throw 'Navigation cancelled'; // Lanzar un error para detener la navegación
      }
    };
    ...
    return () => {
      router.events.off('routeChangeStart', handleRouteChangeStart); // Limpiar el evento al desmontar el componente
    };
  }, [resultado, isNavigating]);

  // Cerrar el toast y permitir la navegación
  const closeToastAndAllowExit = () => {
    setShowToast(false); // Cerrar el toast personalizado
    setIsNavigating(true); // Permitir la navegación
    router.push(router.asPath); // Forzar la navegación una vez que el usuario cierra el toast
  };
  ...
  return (
    <>
      {showToast && <ToastWarningPage onClose={closeToastAndAllowExit} />}
