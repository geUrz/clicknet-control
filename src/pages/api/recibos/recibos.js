...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        ...
        const { usuario_id, folio, cliente_id, tipo_evento, residencial_id } = req.body;

        try {
            const [result] = await connection.query(
                'INSERT INTO recibos (usuario_id, folio, cliente_id, tipo_evento, residencial_id) VALUES (?, ?, ?, ?, ?)',
                [usuario_id, folio, cliente_id, tipo_evento, residencial_id]
            );
        ...
        const { id } = req.query;
        const { cliente_id, tipo_evento } = req.body;

        if (!id) {
            return res.status(400).json({ error: 'ID del recibo es obligatorio' });
        }

        if (tipo_evento && cliente_id) {
            try {
                const [result] = await connection.query(
                    'UPDATE recibos SET cliente_id = ?, tipo_evento = ? WHERE id = ?',
                    [cliente_id, tipo_evento, id]
                );
